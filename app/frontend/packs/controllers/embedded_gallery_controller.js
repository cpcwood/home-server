...
export default class extends Controller {
  ...
  applyBaseStylesToImages() {
    for (let i = 0; i < this.imageTargets.length; i += 1) {
      ...
      }s cubic-bezier(0.76, 0.24, 0.26, 0.99)`
  ...
  quickDisplayCurrentImage() {
    ...
    currentImage.style.transition = `opacity ${
      this.transitionDuration / 1000
    }s cubic-bezier(0.76, 0.24, 0.26, 0.99)`
  }
  ...
  fadeOutOriginalImage(originalPosition) {
    ...
    this.fadeInNextImageTimeout = setTimeout(
      this.fadeInNextImage.bind(this),
      Math.floor(this.transitionDuration * 0.1)
    )
  }
  ...
  fadeInNextImage() {
    ...
    this.transitionCompleteTimeout = setTimeout(
  ...
  set position(newPosition) {
    this.data.set(
      'position',
      Math.abs((newPosition + this.imageTargets.length) % this.imageTargets.length)
    )
  }
}
